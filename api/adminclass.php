...
$qry = 'UPDATE sturollbacktwo SET
	
studentname="'.$studentname.'",rollnumber="'.$rollnumber.'",
checks="'.$checks.'",finalresult="'.$finalresult.'",
status="'.$status.'"
WHERE id ="'.mysqli_real_escape_string($mysqli,$id).'"   ';  
}
...
{
	$date  = date('Y-m-d'); 
	$qry = 'UPDATE  studentrollback  SET status="1"  WHERE sturillid ="'.mysqli_real_escape_string($mysqli,$del).'"'; 
	$res =$mysqli->query($qry)or die("Error in delete query".$mysqli->error);	

	$qry = 'UPDATE  sturollbacktwo  SET status="1"  WHERE sturillid ="'.mysqli_real_escape_string($mysqli,$del).'"'; 
	$res =$mysqli->query($qry)or die("Error in delete query".$mysqli->error);
} 
...
public function getstudentrollback($mysqli,$idupd) {
	$qry = "SELECT * FROM studentrollback WHERE sturollid='".mysqli_real_escape_string($mysqli,$idupd)."'"; 
			...
			$qry = "SELECT * FROM sturollbacktwo WHERE sturollid='".mysqli_real_escape_string($mysqli,$idupd)."'"; 
			$res =$mysqli->query($qry)or die("Error in Get All Records".$mysqli->error);
			$detailrecords = array();
			if ($mysqli->affected_rows>0)
			{
